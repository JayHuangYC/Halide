...
void Blur(std::string suffix, ImageParam input8, const int nChannels, bool vectorized) {
    Var x, y, c;
    Func input;
    ...
    result.compile_to_file(filename + (vectorized? "_vectorized": "") + suffix, args);
}

void Copy(std::string suffix, ImageParam input8, const int nChannels, bool vectorized) {
    Var x, y, c;
    Func input;
    input(x, y, c) = input8(clamp(x, input8.left(), input8.right()),
                            clamp(y, input8.top(), input8.bottom()), c);
