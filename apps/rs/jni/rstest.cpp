...
// void host_blur(buffer_t* in, buffer_t* out) {
//     for (int i = 0; i < in->extent[0]; i++) {
//         for (int j = 0; j < in->extent[1]; j++) {
//             for (int k = 0; k < in->extent[2]; k++) {
//                 // char buffer [512];
//                 // sprintf(buffer, "(%d+%d+%d)/3=",
//                 //     in->host[i*in->stride[0] + j*in->stride[1] + k*in->stride[2]],
//                 //     in->host[std::max(0, std::min(i+1, in->extent[0]-1))*in->stride[0] + j*in->stride[1] + k*in->stride[2]],
//                 //     in->host[std::max(0, std::min(i+2, in->extent[0]-1))*in->stride[0] + j*in->stride[1] + k*in->stride[2]]);
//                 // std::cout << buffer;
//                 out->host[i*out->stride[0] + j*out->stride[1] + k*out->stride[2]] =
//                     round((in->host[i*in->stride[0] + j*in->stride[1] + k*in->stride[2]] +
//                     in->host[std::max(0, std::min(i+1, in->extent[0]-1))*in->stride[0] + j*in->stride[1] + k*in->stride[2]] +
//                     in->host[std::max(0, std::min(i+2, in->extent[0]-1))*in->stride[0] + j*in->stride[1] + k*in->stride[2]])/3.0f);
//                 // sprintf(buffer, "%d", out->host[i*out->stride[0] + j*out->stride[1] + k*out->stride[2]]);
//                 // std::cout << buffer << "\n";
//             }
//         }
//     }
    ...
    for (int i = 0; i < actual.extent[0]; i++) {
        for (int j = 0; j < actual.extent[1]; j++) {
            for (int k = 0; k < actual.extent[2]; k++) {
                uint8_t actual_value = actual.host[i * actual.stride[0] +
                                   j * actual.stride[1] +
                                   k * actual.stride[2]];
                uint8_t expected_value = expected.host[i * expected.stride[0] +
                                   j * expected.stride[1] +
                                   k * expected.stride[2]];
                if (actual_value != expected_value) {
                    if (count_mismatches < 100) {
                        char buf[512];
                        sprintf(buf, "actual and expected results differ at (%d, %d, %d): %d != %d\n",
...
buffer_t make_planar_image(int width, int height, int channels, uint8_t host[]) {
    buffer_t bt_input = { 0 };
...
bool test(buffer_t bt_input, buffer_t bt_output, buffer_t bt_output_arm,
        ...
        int (generated_arm) (buffer_t *_input_buffer, buffer_t *_result_buffer)) {
    ...
    double t_rs = (t2.tv_sec - t1.tv_sec)*1000.0 + (t2.tv_usec - t1.tv_usec)/1000.0;
    if (error) {
    ...
    gettimeofday(&t2, NULL);
    double t_arm = (t2.tv_sec - t1.tv_sec)*1000.0 + (t2.tv_usec - t1.tv_usec)/1000.0;
    if (error) {
        std::cout << "Halide returned error: " << error << std::endl;
    }
    printf("Ran %d reps. One rep times:\nRS:  %fms\nARM: %fms\n", n_reps, t_rs/n_reps, t_arm/n_reps);
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < std::min(bt_input.extent[0], width); i++) {
        for (int j = 0; j < std::min(bt_input.extent[1], height); j++) {
            for (int k = 0; k < bt_input.extent[2]; k++) {
                input_image[i * bt_input.stride[0] + j * bt_input.stride[1] + k * bt_input.stride[2]] =
                    ((i + j) % 2) * 6;
            }
        }
    }
    buffer_t bt_output = make_planar_image(width, height, channels, output_image);
    buffer_t bt_output_arm = make_planar_image(width, height, channels, output_image_arm);

    std::cout << "Planar blur:\n";
    bool correct = true;
    if (!test(bt_input, bt_output, bt_output_arm, generated_blur_rs, generated_blur_arm)) {
        correct = false;
    }
    std::cout << "Planar copy:\n";
    if (!test(bt_input, bt_output, bt_output_arm, generated_copy_rs, generated_copy_arm)) {
        correct = false;
    }

    buffer_t bt_interleaved_input = make_interleaved_image(width, height, channels, input_image);
    for (int i = 0; i < std::min(bt_interleaved_input.extent[0], width); i++) {
        for (int j = 0; j < std::min(bt_interleaved_input.extent[1], height); j++) {
            for (int k = 0; k < bt_interleaved_input.extent[2]; k++) {
                input_image[i * bt_interleaved_input.stride[0] + j * bt_interleaved_input.stride[1] + k * bt_interleaved_input.stride[2]] =
                    ((i + j) % 2) * 6;
            }
        }
    }
    buffer_t bt_interleaved_output = make_interleaved_image(width, height, channels, output_image);
    buffer_t bt_interleaved_output_arm = make_interleaved_image(width, height, channels, output_image_arm);

    std::cout << "\nInterleaved(vectorized) blur:\n";
    if (!test(bt_interleaved_input, bt_interleaved_output, bt_interleaved_output_arm,
              generated_blur_vectorized_rs, generated_blur_vectorized_arm)) {
        correct = false;
    }
    std::cout << "\nInterleaved(vectorized) copy:\n";
    if (!test(bt_interleaved_input, bt_interleaved_output, bt_interleaved_output_arm,
              generated_copy_vectorized_rs, generated_copy_vectorized_arm)) {
        correct = false;
    }
